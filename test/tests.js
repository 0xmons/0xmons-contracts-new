...
contract("monMinter tests", async accounts => {
  it ("allows setting all of the variables", async() => {
    ...
    // ensure admin role can call grantRole of minter to itself
    // ensure admin role can call grantRole of minter to monStaker contract
    ...
    await monMinter.mintMonster(accounts[0], 0, 0, 0,  0, 0, 0, {from: accounts[0]});
    await truffleAssert.reverts(
      monMinter.mintMonster(
        accounts[0], 0, 0, 0,  0, 0, 0, {from: accounts[1]})
    );
    ...
    await monMinter.modifyMon(0, false, 1, 11, 2, 3, 4, 5, {from: accounts[0]});
    let mon0 = await monMinter.monRecords(0);
    expect(mon0["summoner"]).to.eql(accounts[0]);
    expect(mon0["parent1"]).to.eql(web3.utils.toBN(1));
    expect(mon0["parent2"]).to.eql(web3.utils.toBN(11));
    ...
    await monMinter.modifyMon(0, true, 0, 0, 0, 0, 0, 10, {from: accounts[0]});
    ...
    await monMinter.modifyMon(0, true, 10, 110, 20, 30, 40, 50, {from: accounts[0]});
    mon0 = await monMinter.monRecords(0);
    expect(mon0["parent1"]).to.eql(web3.utils.toBN(10));
    expect(mon0["parent2"]).to.eql(web3.utils.toBN(110));
    ...
    await truffleAssert.reverts(
      monMinter.modifyMon(0, false, 0, 0, 2, 3, 4, 5, {from: accounts[1]})
    );
...
// stake -> redeem for monster -> get two, spawn

// Ensure only admin can change the extension URI

// Ensure only admin can change the spawnFee

// Ensure only admin can change the default spawn delay

// Ensure two monsters can only spawn together once